...
class DrawAC(Node):
    def __init__(self):
        ...
        self.starting_position = [-4.0 , -9.0]
        self.starting_angle = 0.0

        #Publishers
        self.twist_pub = self.create_publisher(Twist, 'Loki/twist_mux/cmd_vel', 1)
        self.twist_2_pub = self.create_publisher(Twist, 'Odin/twist_mux/cmd_vel', 1)
    ...
    def drawing_c(self):
        ...
        twist_2_msg.linear.x = -(self.vx * 5)
        twist_2_msg.linear.y = self.vy * 5
        twist_2_msg.linear.z = 0.0
        
        twist_2_msg.angular.x = 0.0
        twist_2_msg.angular.y = 0.0
        twist_2_msg.angular.z = -(self.wz * 5)

        self.twist_pub.publish(twist_@_msg)
